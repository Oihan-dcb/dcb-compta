...
| `reservation.owner_stay === true` | Exclue — séjour propriétaire. Guard explicite dans `calculerVentilationResa` : `ventilation_calculee=true`, aucune ligne produite. La ventilation est saisie manuellement via `VentilationEdit` (codes FMEN + AUTO), ou via le bouton « à saisir » de PageRapports qui pose une ligne **MEN** (« Ménage séjour propriétaire », `calcul_source='manual'`) = total ménage refacturé au proprio, coût AE hors TVA. Jamais FMEN pour cette saisie : le FMEN part dans la facture Evoliz forfait ménage. Le MEN owner-stay suit le **canal AUTO** de l'absorption LOY (facturesEvoliz + buildComptaMensuelle) et rend le séjour absorbable même si `fin_revenue` est null ; il est exclu du `menBien` (filet ménage voyageur) dans facturesEvoliz mais conservé dans `menHt` de buildComptaMensuelle (dont l'autoHt vient des missions et inclut la mission owner-stay). | ✅ Session 04/07/2026 |
...
|---|---|---|
| Cleaning fee (Airbnb) | `label === 'cleaning fee'` (égalité stricte, après normalisation) | `cleaningFeeAirbnb` — frais ménage facturés au voyageur sur Airbnb |
| Community fee | `label === 'community fee'` (égalité stricte, après normalisation) | `communityFeeRaw` — commission Airbnb sur l'hébergement (Airbnb) ou ménage direct (Hospitable) |
...
**Normalisation des labels localisés (session 04/05/2026)** : Booking peut retourner les labels de fees dans la langue du compte. Le code normalise les labels au moment du fallback `hospitable_raw` (table `LABEL_ALIASES` dans `_calculerLignes`) :

| Label source (localisé) | Label normalisé | Plateforme |
|---|---|---|
| `"frais de ménage"` | `"cleaning fee"` | Booking FR |
| `"frais de service (5%)"` | `"community fee"` | Booking FR |

Cette normalisation s'applique **uniquement** au fallback `hospitable_raw` (quand `reservation_fee` est vide). Les lignes déjà en base dans `reservation_fee` ne sont pas concernées — elles ont les labels au moment de l'import CSV.
...
|---|---|---|
| Labels fees localisés (Booking) | ✅ **Mitigé** session 04/05/2026 : normalisation `LABEL_ALIASES` FR→EN dans `_calculerLignes`. Migration 124 recalcule 102 resas Booking affectées. Autres langues : ajouter à `LABEL_ALIASES` si besoin. | FMEN absent → 0 |
| Labels fees instables (Airbnb, Direct) | Si Hospitable change "Cleaning fee" → `cleaningFeeAirbnb = 0` silencieusement | FMEN et LOY faux |
...
Le mot "VIR" désigne quatre objets distincts dans ce projet. **Toujours utiliser le nom officiel ci-dessous** dans les commits, commentaires, conversations et code. Ne jamais écrire "VIR" seul sans qualificatif.

---

### VIRProprio : code comptable `ventilation.code = 'VIR'`

**Ce que c'est** : ligne de ventilation produite par `ventilation.js`. Représente le montant que DCB doit reverser au propriétaire après ses retenues.

**Formule** : `VIRProprio = LOY + taxesTotal`

**Table** : `ventilation` (code='VIR', montant en centimes)

**Remarque** : c'est un montant *calculé*, pas un flux bancaire. Il peut exister sans mouvement bancaire associé.
...
**Ce que c'est** : un VIRProprio qui a été *rapproché* à un mouvement bancaire réel. Prouve que le payout Airbnb/Booking est arrivé en banque et a été comptabilisé.

**Condition** : `ventilation.code = 'VIR'` ET `mouvement_id IS NOT NULL` ET `mouvement_bancaire.date_operation` connue.

**Usage clé** : c'est ce VIRPayinProuvé que `SequestreCloture` utilise pour qualifier une réservation Airbnb/Booking comme **Certain**. Formule : `VIRPayinProuvé.mouvement.date_operation <= dateCloture`.

**Alias dans le code** : `virProuve`, `virs.some(v => v.mouvement?.date_operation ...)`

---

### PAYIN : mouvement bancaire entrant (libellé contenant "VIR SEPA")

**Ce que c'est** : un `mouvement_bancaire` entrant provenant d'Airbnb, Booking, Stripe ou d'un voyageur direct. Libellé type : `"VIR SEPA AIRBNB PAYMENTS LUXEMB"`, `"VIR SEPA BOOKING.COM"`.

**Table** : `mouvement_bancaire` (`canal = 'airbnb'` | `'booking'` | `'stripe'` | `'sepa_manuel'`)

**Remarque** : c'est la *source* qui permet de créer VIRPayinProuvé (via rapprochement). Avant rapprochement, c'est un mouvement isolé sans lien avec une réservation.

**Alias dans le code** : "mouvement bancaire entrant", `mouvement_bancaire`, "payout bancaire"

---

### VIRProprioRéel : VIR trésorerie (matrice de contrôle)

**Ce que c'est** : montant *résiduel* calculé dans la matrice de contrôle trésorerie par bien. Ne correspond ni à un code comptable ni à un flux bancaire.

**Formule** : `max(0, creditsProuves − HON_ttc − FMEN_ttc − AUTOREEL − COM_ttc − PREST − HAOWNER)`

**Usage** : indicateur de ce que DCB peut réellement virer au propriétaire après ses retenues, basé sur les fonds bancaires effectivement reçus (pas sur `fin_revenue` brut).

**Alias dans le code** : `fraisVIR`, colonne "VIR" dans la matrice de contrôle

---

### Tableau récapitulatif

| Nom officiel | Table / Champ | Signification | Contexte d'usage |
|---|---|---|---|
| **VIRProprio** | `ventilation.code='VIR'` | Montant calculé à reverser au proprio | Facturation, exports SEPA, rapports |
| **VIRPayinProuvé** | VIRProprio + `mouvement_id IS NOT NULL` | Payout plateforme reçu et rapproché | `SequestreCloture` — condition "Certain" Airbnb/Booking |
| **PAYIN** | `mouvement_bancaire` | Virement bancaire entrant brut | Rapprochement bancaire, PageBanque |
