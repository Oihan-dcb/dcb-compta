...
function buildBlocC(mouv, resa, resaIds, codes, resaDetails, payoutByMouv) {
  ...
  return {
    ...
    Plateforme:                   platform,
    'Check-in':                   formatDateFR(resa?.arrival_date  || first.arrival_date),
    'Check-out':                  formatDateFR(resa?.departure_date || first.departure_date),
    Nuits:                        resa?.nights ?? first.nights ?? '',
    'Code résa':                  codes.join(' | '),
    reservation_status_normalise: statusNorm,
    is_owner_stay:                first.owner_stay           ?? false,
    is_cancelled:                 statusNorm === 'cancelled',
    is_manual_import:             platform   === 'manual',
    is_direct_booking:            platform   === 'direct',
    is_grouped_payout:            nbResas    >  1,
...
function buildHeaders() {
  ...
  return [
    ...
    // BLOC C (17)
    'Bien(s)', 'Voyageur(s)', 'Plateforme', 'Check-in', 'Check-out', 'Nuits', 'Code résa',
    'reservation_status_normalise', 'is_owner_stay', 'is_cancelled', 'is_manual_import',
    'is_direct_booking', 'is_grouped_payout', 'nb_reservations_liees', 'codes_resas_lies',
