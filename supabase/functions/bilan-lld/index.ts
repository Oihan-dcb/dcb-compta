  ...
  try {
    ...
    const [{ data: agency }, { data: loyers }, { data: virements }] = await Promise.all([
      ...
      supabase.from('loyer_suivi')
        .select('*, etudiant!inner(nom, prenom, loyer_nu, supplement_loyer, charges_eau, charges_copro, charges_internet, bien(code))')
        .eq('agence', agence).eq('mois', mois)
        .eq('etudiant.archived', false)
        .order('created_at'),
      supabase.from('virement_proprio_suivi')
        .select('*, etudiant!inner(nom, prenom, proprietaire(nom, prenom))')
        .eq('agence', agence).eq('mois', mois)
        .eq('etudiant.archived', false)
        .order('created_at'),
    ])
