...
// DCB uniquement (Lauïan n'a pas son séquestre sur Pennylane) : ignoré sur lauian-compta.
...
function htmlAlerte(j, precedent, nouvelles) {
  ...
  <table width="680" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;max-width:680px;width:100%">
    <tr><td style="background:#CC9933;padding:20px 24px;color:#fff;text-align:center">
      ...
      <div style="font-size:18px;font-weight:bold;margin-top:6px">Séquestre — justificatif du ${j.date.split('-').reverse().join('/')}</div></td></tr>
...
export default async function handler(req, res) {
  ...
  if (AGENCE !== 'dcb') return res.status(200).json({ ok: true, skipped: 'sequestre_pennylane_dcb_only', agence: AGENCE })

  const dryRun = req.query?.dry_run === '1'
  try {
    const j = await justifierSequestre('dcb')
    if (dryRun) return res.json({ ok: true, dry_run: true, ...j })

    const { data: precedent } = await supabase.from('sequestre_justificatif')
      .select('date, ecart, detail').eq('agence', 'dcb').lt('date', j.date).order('date', { ascending: false }).limit(1).maybeSingle()
    const { error } = await supabase.from('sequestre_justificatif').upsert({
      agence: 'dcb', date: j.date, solde_banque: j.solde_banque.montant, solde_maj: j.solde_banque.maj,
      total_justifie: j.total_justifie, ecart: j.ecart, poches: j.poches, par_mois: j.par_mois,
      detail: { ...j.detail, anomalies: j.anomalies },
    ...
    if (aBouge || nouvelles.length || !precedent) {
      const r = await fetch(`${SUPABASE_URL}/functions/v1/smtp-send`, {
        ...
        body: JSON.stringify({ to: ALERTE_TO, subject: `Séquestre : écart ${eur(j.ecart)}${aBouge ? ` (${j.ecart - precedent.ecart > 0 ? '+' : ''}${eur(j.ecart - precedent.ecart)})` : ''}${nouvelles.length ? ` — ${nouvelles.length} nouvelle(s) anomalie(s)` : ''}`, html: htmlAlerte(j, precedent, nouvelles) }),
      })
    ...
    await supabase.from('journal_ops').insert({ categorie: 'banque', action: 'sequestre_justificatif', source: 'cron', statut: Math.abs(j.ecart) > 100 ? 'warning' : 'ok',
      message: `Séquestre ${j.date} : solde ${eur(j.solde_banque.montant)}, justifié ${eur(j.total_justifie)}, écart ${eur(j.ecart)}, ${j.anomalies.length} anomalie(s)` })
    return res.json({ ok: true, date: j.date, solde: j.solde_banque.montant, justifie: j.total_justifie, ecart: j.ecart, anomalies: j.anomalies.length, alerte })
  } catch (err) {
    console.error('[sequestre-justificatif]', err.message)
    await supabase.from('journal_ops').insert({ categorie: 'banque', action: 'sequestre_justificatif', source: 'cron', statut: 'error', message: err.message }).catch(() => {})
