...
export default async function handler(req, res) {
  ...
  try {
    const { action, ae_id, email: aeEmail, mois, actif, redirect_to } = req.body
    if (!action || !ae_id) return res.status(400).json({ error: 'action et ae_id requis' })

    let slug, payload
    if (action === 'create') { slug = 'create-ae-user'; payload = { ae_id, email: aeEmail, redirect_to } }
    else if (action === 'reset') { slug = 'reset-ae-password'; payload = { ae_id, redirect_to } }
